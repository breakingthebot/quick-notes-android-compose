...
- I polished the visual design of the screen layout to reduce heavy visual clutter and dark card blocks. The summary header has been made completely borderless with vibrant theme primary colored accents, and all control cards (folders, tags, filters, and note editor) have been styled with clean surface backgrounds, thin outline borders, and matching primary-colored titles. Default note cards now use clean, outlined borders on top of default surface colors instead of heavy grey solid backgrounds.
- I added Smart AI Voice Notes with offline Speech-to-Text transcription, auto-tagging, automatic notebook classification, and natural language date extraction. Using Android's native `SpeechRecognizer` and `MediaRecorder` APIs, the app records private audio files and transcribes voice notes completely offline. The `SmartContentAnalyzer` heuristic NLP engine automatically extracts tags, classifies notes into folders by matching names/synonyms, and detects date/time phrases (like "tomorrow at 6 pm" or "in 3 hours") to schedule native alarms. The database version was bumped to 11 with migrations to support `audioUri` columns.

The code is split into small files by responsibility: Room entities and DAO in `data` and `model`, formatting and input cleanup in `util`, screen rendering in `ui`, widget rendering and snapshot loading in `widget`, mutation side effects in `services`, and state management in `viewmodel`. Search, collection scoping, tag filtering, and sorting are handled as pure list-formatting logic in the state layer instead of being mixed into the database contract, while the widget reads a small recent-note snapshot from the same Room database and refreshes through a notifier after note mutations. That notifier now does both a Glance refresh and an explicit widget-update broadcast so launcher hosts repaint reliably. Database changes are managed via version 11 migrations that add the `Notebook` entity, the `notebookId` column, the `imageUri` column, and the `audioUri` column to notes. The presentation layer still keeps a clean split between the pure `QuickNotesScreen` composable and the `QuickNotesApp` view-model wrapper, and the CI workflow runs `testDebugUnitTest` for logic and Robolectric-backed UI coverage.
